...
- [What are three reasons why you may see a TypeError?](#type-error) - One method that you can use when resolving an error is by reading what the error says, navigating to the error line, & seeing what type of error is being thrown, while also using breakpoints or console.log to see how the program works 
- [What is the key difference between an error and a warning?](#errors-vs-warnings) - A warning doesn’t stop the program from running, While a warning lets the program run with the possibility that the program won’t run perfectly
- [What is one method you can use to resolve an error?](#tips-for-resolving-errors) - One method that you can use when resolving an error is by reading what the error says, navigating to the error line, & seeing what type of error is being thrown, while also using breakpoints or console.log to see how the program works 
